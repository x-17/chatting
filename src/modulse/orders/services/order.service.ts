...
export class OrderService {
    ...
    async createOrderWithConversation(data: CreateOrderRequest): Promise<{
        ...
        try {
            ...
            console.error('[OrderService] Create order with conversation failed:', error);
            throw error;
        }
    }
    ...
    async getOrderWithConversation(orderId: string): Promise<{
        ...
        try {
            ...
            console.error('[OrderService] Get order with conversation failed:', error);
            throw error;
        }
    }
    ...
    private async ensureConversation(order: Order): Promise<boolean> {
        try {
            if (order.conversationType === 'p2p') {
                ...
                // 确保与对方的加密会话已建立
                const otherUserId = order.buyerId === this.myUserId
                    ? order.sellerId
                    : order.buyerId;
    ...
    async markOrderMessagesAsRead(orderId: string): Promise<void> {
        try {
            ...
            if (order.conversationType === 'p2p') {
                ...
                const messages = await p2pRouter.getConversationHistory(otherUserId, 100);
    ...
    getOtherParty(order: Order): {
        ...
        const otherParty = order.participants.find(p =>
            p.role === (isBuyer ? 'seller' : 'buyer')
        );
