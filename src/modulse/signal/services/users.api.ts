...
export class UsersApiService {
    ...
    async uploadKeyBundle(bundle: PublicKeyBundle): Promise<{
        ...
        try {
            ...
            throw new Error(`上传密钥失败: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    async getKeyBundleForUser(tenantId: string) {
        try {
            ...
            console.log(`[UsersApi] 成功获取用户 ${tenantId} 的密钥束，用户ID: ${publicKeyBundle.userId}`);
            ...
            throw new Error(`获取用户密钥失败: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     * 批量获取多个用户的公钥束（后续备用）
    ...
    async getKeyBundlesForUsers(userIds: string[]): Promise<{
        success: boolean;
        bundles: { [userId: string]: PublicKeyBundle & { lastUpdated: number; isActive: boolean } };
        failed: string[];
    }> {
        try {
            const response = await this.apiClient.post<{
            ...
            console.log(`[UsersApi] Retrieved key bundles for ${Object.keys(response.data.bundles).length} users, failed: ${response.data.failed.length}`);
            return response.data;
     ...
     */
    async updateKeyBundle(bundle: Partial<PublicKeyBundle> & { userId: string }): Promise<{
        success: boolean;
        timestamp: number;
    }> {
        try {
            ...
            throw new Error(`更新密钥失败: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    async getOrderParticipants(orderId: string): Promise<{
        ...
        try {
            ...
            throw new Error(`获取订单信息失败: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    async getUserOrders(userId: string, page: number = 1, limit: number = 20): Promise<{
        ...
        try {
            ...
            console.log(`[UsersApi] Retrieved ${response.data.orders.length} orders for user: ${userId}`);
            return response.data;
    ...
    async checkUserOnlineStatus(userIds: string[]): Promise<{
        ...
        try {
            ...
            console.log(`[UsersApi] Checked online status: ${response.data.online.length} online, ${response.data.offline.length} offline`);
            return response.data;
    ...
    async searchUsers(query: string, limit: number = 10): Promise<{
        ...
        try {
            ...
            console.log(`[UsersApi] Found ${response.data.users.length} users for query: ${query}`);
            return response.data;
...
export async function getKeyBundleForUser(tenantId: string): Promise<PublicKeyBundle> {
...
export async function updateKeyBundle(bundle: Partial<PublicKeyBundle> & { userId: string }): Promise<{
    success: boolean;
    timestamp: number;
}> {
...
export async function getUserOrders(userId: string, page?: number, limit?: number): Promise<{
    orders: {
        orderId: string;
        title: string;
        status: string;
...
export async function searchUsers(query: string, limit?: number): Promise<{
    users: {
        userId: string;
        username: string;
        avatar?: string;
