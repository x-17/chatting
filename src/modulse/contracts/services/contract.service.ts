...
import type { Contract, SignatureRecord, ContractParticipant } from '../types/contract.types';
...
export class ContractService {
    ...
    async signContract(contractId: string): Promise<{
        ...
        try {
            ...
            // 6. 提交到服务器
            const result = await this.apiService.submitSignature(contractId, signatureRecord);
    ...
    async verifyContract(contractId: string): Promise<{
        ...
        try {
            ...
            for (const participant of contract.participants) {
                ...
                // 获取签名者的公钥
                const publicKeys = await e2eeService.getUserPublicKeys(participant.userId);
                if (!publicKeys) {
    ...
    private validateSigningPermission(contract: Contract): void {
        // 检查状态
        if (contract.status !== 'pending_signatures' && contract.status !== 'signing') {
