...
export function useChat() {
        ...
        try {
            ...
            if (mockService.enabled) {
                ...
                if (order.conversationType === 'p2p') {
                    const mockMsgs = await mockService.getP2PMessages(order.conversationId);
                    messages = mockMsgs.map(msg => ({
                        ...msg,
                        __conversationType: 'p2p' as const
                    }));
                } else if (order.conversationType === 'group') {
                    const mockMsgs = await mockService.getGroupMessages(order.conversationId);
                    messages = mockMsgs.map(msg => ({
                        ...msg,
        ...
        try {
            ...
            if (mockService.enabled) {
                const optimisticMessage: ChatMessage = {
                    ...
                    type: 'text',
                    senderId: myUserId,
                    recipientId: order.conversationType === 'p2p' ? order.otherParty.id : undefined,
                    groupId: order.conversationType === 'group' ? order.conversationId : undefined,
                    content: content,
                    timestamp: Date.now(),
     ...
     * 发送文件消息（Mock 版本）
     */
    async function sendFile(order: Order, file: File): Promise<void> {
